...
#
# 2) To also download a zip file containing residuals, add --res command line flag.
#
# 3) If you find that the script keeps timing out, try increasing the value of --get_max.
#
# 4) Use --web command line flag to visit CSRS-PPP website (provided a web browser is installed).
#
# CHANGELOG
# ---------
# DATE          WHO						DESCRIPTION
# 2019-05-30    Justin Farinaccio		1.4.0
# Modified:
#   2019-05-30 - 1.4.0 - JF
#       Added res as an optional command line flag to download residuals zip file
#       Gave get_max an upper limit to wait no more than 30 for results
...
__copyright__ = 'Copyright (c) 2017-2019, Justin Farinaccio,' \
...
parser = argparse.ArgumentParser(
    description='{0}'.format('# ---------------- Using CSRS-PPP via script, version 1.4.0 ---------------- #'),
...
if args.web:
    try:
        ...
        time.sleep(3)
        webbrowser.open('https://webapp.geod.nrcan.gc.ca/geod/tools-outils/ppp.php?locale={0}'.format(args.lang),
...
for request_num in range(request_max):
    ...
    if req.text:
        ...
        while not (procstat == 'done' or error == 1):
            ...
            # get 'Status.txt'
            r = requests.get('https://webapp.geod.nrcan.gc.ca/CSRS-PPP/service/results/status?id={0:s}"'.format(keyid),
                             timeout=5)
            ...
            # Re-get!
                ...
                print('=> get[{0:d}]: "Status.txt" *NOT* found! get again!'.format(get_num))
        ...
        if not error:
            ...
            for get_num in range(args.get_max):
                # get
                r = requests.get('https://webapp.geod.nrcan.gc.ca/CSRS-PPP/service/results/file?id={0:s}'.format(keyid),
                                 timeout=5)
                ...
                for get_num in range(args.get_max):
                    ...
                    try:
                        ...
                        for extract_name in zipfile.ZipFile(rinex_file, 'r').namelist():
                            ...
                            # get
                            r = requests.get('https://webapp.geod.nrcan.gc.ca/CSRS-PPP/service/results/file?id={0:s}&fid={1:02d}&type=res'.format(keyid, num_files),
                                             timeout=5)
